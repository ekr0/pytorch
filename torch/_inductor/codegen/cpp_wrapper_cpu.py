...
class DeferredCpuTritonCallWrapper:
    """
    CPU AOTI counterpart to DeferredTritonCallWrapper in cpp_wrapper_gpu.py.

    Deferred because pass-2 codegen needs CpuTritonKernelCache entries that
    only get populated when pass-1's autotune block runs Triton CPU
    compilation. Constructed at the kernel call site; `generate(wrapper)` is
    invoked from `finalize_prefix`.
    """
    ...
    def _get_cpp_param_type(self, name: str, arg_type: Any) -> str:
        # Mirrors DeferredTritonCallWrapper._get_cpp_param_type (GPU).
    ...
    def _resolve_arg_names(self) -> tuple[list[str], list[str]]:
        """Return (kernel_arg_names, grid_arg_names).

        kernel_arg_names get packed into `void** args`; grid_arg_names get
        passed directly as `gridX/gridY/gridZ` to `run_from_nativert`.
        """
    ...
    def _write_wrapper_signature(
        ...
        # Reuses GPU's `cubin_dir_` parameter name as the .so-dir override so
        # both wrappers can be driven by the same AOTI runtime member.
    ...
    def _generate_launch(
        ...
        # Triton CPU launcher ABI: args[i] is the data ptr for `*T` args, or
        # the address of the scalar value otherwise.
        kernel_member = f"kernels_.{self.kernel_name}_kernel_fn"
        launcher_member = f"kernels_.{self.kernel_name}_launcher_fn"
        ...
        args_arr_str = ", ".join(args_array_entries) if args_array_entries else ""

        # Grid args come from extra_launcher_args; they're typed int64_t in
        # the wrapper signature but the launcher ABI expects uint32_t.
    ...
    def generate(self, wrapper: CppWrapperCpu) -> None:
        ...
        if info is None:
            raise RuntimeError(
                f"CPU AOTI: CpuTritonKernelCache not populated for kernel "
                f"'{self.kernel_name}'. Pass-1 should have executed the "
                "autotune block and triggered Triton CPU compilation. Verify "
                "config.triton.autotune_at_compile_time is True (default for "
                "AOTI) and that the active Triton CPU backend emits a "
                "launcher .so exporting `run_from_nativert`."
            )

        signature = info["signature"]
        kernel_arg_names, grid_arg_names = self._resolve_arg_names()
        wrapper_arg_names = kernel_arg_names + grid_arg_names
...
class CppWrapperCpu(PythonWrapperCodegen):
    ...
    def __init__(self):
        ...
        # `_names`: set of *defined* Triton kernels (drives member
        # declarations on `AOTInductorModelKernels`). `_call_wrappers`: dict of
        # *called* kernels (drives `static void call_<k>(...)` emission in
        # `finalize_prefix`). A kernel can be defined without being called.
    ...
    def _generate_kernel_call_helper(
        ...
        *,
        ...
        # CPU AOTI Triton kernels: route via the autotune block (so pass-1
        # populates CpuTritonKernelCache) and emit a `call_<k>(...)` to a
        # deferred wrapper finalize_prefix will define. Mirrors the GPU flow
        # in cpp_wrapper_gpu.py:_generate_kernel_call_helper.
        if triton and kernel_name in self._cpu_triton_kernel_names:
            ...
            # Mirrors prepare_triton_wrapper_args in cpp_wrapper_gpu.py.
            stringified_args = [
    ...
    def finalize_prefix(self):
        ...
        # Mirrors cpp_wrapper_gpu's `finalize_prefix` Triton-wrapper emission.
        # Includes are emitted whenever a kernel was *defined* (the model
        # kernels class references `CpuTritonLauncherFn`); definitions only
        # for kernels that were *called*.
    ...
    def _stringify_cpu_triton_call_arg(arg: Any) -> str:
        ...
        Handles the same arg shapes as
        `CppWrapperGpu.prepare_triton_wrapper_args`: tensor variable names
        come through as `str`; bool/int/float become C++ literals; sympy
        expressions go through `cexpr` after sizevar simplification.
        """
    ...
    def _define_kernel_helper(
        ...
        # Misnomer: `gpu` actually means "this is a Triton kernel"
        gpu: bool = False,
        cpp_definition: str | None = None,
    ):
        if gpu:
            # The kernel body is Python (`async_compile.triton(...)`); clang
            # can't parse it, so we route it to the autotune block (which
            # pass-1 executes to populate CpuTritonKernelCache) instead of
            # splicing into the C++ header.
